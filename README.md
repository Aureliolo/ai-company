...
Per-agent and shared organizational memory with hybrid retrieval pipeline (dense + BM25 sparse with RRF fusion), tool-based and context injection strategies, non-inferable filtering, consolidation, archival, and procedural memory auto-generation from agent failures (SKILL.md materialization). Pluggable backends via protocol.
