...
| **Agent Memory** | Mem0 (Qdrant + SQLite) initially, custom stack (Neo4j + Qdrant) planned | Mem0 runs in-process as the initial backend behind a pluggable `MemoryBackend` protocol ([Decision Log](decisions.md)). Hybrid dense + BM25 sparse retrieval with RRF fusion (opt-in via `sparse_search_enabled`). `mmh3` for vocabulary-free murmurhash3 token hashing; Qdrant `Modifier.IDF` for server-side IDF scoring. Qdrant embedded + SQLite for persistence. Custom stack as a future upgrade. Config-driven backend selection. |
