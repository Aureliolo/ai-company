...
| **Web UI** | Vue 3 + Vite + PrimeVue + Tailwind CSS | Modern, fast, good ecosystem. Simpler than React for dashboards. Pinia state management, vue-router routing, Axios HTTP client, VueFlow org chart visualization, ECharts + vue-echarts budget/spending charts, PrimeVue UI component library, Tailwind CSS utility-first styling, CodeMirror 6 code editor (vue-codemirror), js-yaml for YAML support, @vitejs/devtools (dev mode only). |
