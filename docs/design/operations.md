...
| `/api/v1/auth` | Authentication: setup, login (HttpOnly cookie sessions, CSRF double-submit), password change (rotates session cookie), ws-ticket, session management (list/revoke, concurrent session limits), logout (idempotent 204; always emits `Max-Age=0` `Set-Cookie` headers for the three session cookies -- `session`, `csrf_token`, `refresh_token` -- plus `Clear-Site-Data: "cookies"`; excluded from both auth and CSRF middleware so clients can recover from stale cookie state; server-side JTI revocation is best-effort when a valid JWT is presented), account lockout (429 with Retry-After), refresh token rotation (three-tier rate limiting: 10,000 req/min/IP floor, 20 req/min unauth by IP, 6,000 req/min auth by user ID -- see `docs/security.md`) |
