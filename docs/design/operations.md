...
| `/api/v1/auth` | Authentication: setup, login (HttpOnly cookie sessions, CSRF double-submit), password change (rotates session cookie), ws-ticket, session management (list/revoke, concurrent session limits), logout (idempotent 204 with `Clear-Site-Data`; excluded from auth middleware and CSRF so clients can recover from stale cookie state), account lockout (429 with Retry-After), refresh token rotation (three-tier rate limiting: 300 req/min/IP floor, 20 req/min unauth by IP, 6,000 req/min auth by user ID -- see `docs/security.md`) |
