...
| `/api/v1/analytics` | `GET /overview` (metrics summary with budget status, 7d spend sparkline, agent counts), `GET /trends` (time-series bucketed metrics: spend, tasks, success rate, active agents), `GET /forecast` (budget spend projection with exhaustion estimate) |
