...
???+ note "Provider Configuration (YAML)"
    ...
    providers:
      example-provider:
        ...
        # subscription_token: "..."    # subscription token (subscription auth only; passed to LiteLLM as api_key -- auto-detected as OAuth by token prefix; sensitive -- use env vars or secret management)
...
- **Auth types**: `api_key` (default), `subscription` (token-based auth for provider subscription plans, passed to LiteLLM as `api_key` -- auto-detected as OAuth by token prefix, requires ToS acceptance), `oauth` (stores credentials, MVP uses pre-fetched token), `custom_header`, `none` (local providers)
