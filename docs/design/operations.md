...
- **Model discovery**: `POST /api/v1/providers/{name}/discover-models?preset_hint={preset_name}` -- queries the provider endpoint for available models (Ollama `/api/tags`, standard `/models`) and updates the provider config. The optional `preset_hint` query parameter guides endpoint selection and enables SSRF bypass for trusted preset URLs (e.g. `host.docker.internal`). Also auto-triggered on preset creation for no-auth providers with empty model lists. SSRF-safe URL validation applied before outbound requests; bypassed for trusted URLs (no-auth local providers or validated preset-hinted URLs where `base_url` matches the preset's `candidate_urls`) where localhost/private IPs are expected by design. SSRF bypass is logged at WARNING level (`PROVIDER_DISCOVERY_SSRF_BYPASSED`).
