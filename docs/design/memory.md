...
progress markers (`STAGE_START:`, `STAGE_COMPLETE:`) on stdout that the orchestrator
parses from container logs. Source data is mounted at `/data` (read-only), checkpoints
written to `/checkpoints` (read-write). GPU passthrough is available via
`gpu_enabled=True`. The in-process fallback (`backend="in-process"`) is preserved for
non-Docker deployments where torch is installed directly.
