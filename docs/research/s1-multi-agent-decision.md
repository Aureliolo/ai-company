...
|---|---|---|---|
| **Per-coordination-group** (agents working on a single `coordination_topology` wave) | **3-4 active agents** | Kim 2025 180-experiment cap; per-agent reasoning degrades sharply beyond this | **Soft cap** -- default enforced in `CoordinationConfig`, override via explicit config flag with justification. Legitimate 5-6 sub-agent decompositions exist in the +57% to +81% parallel regime. |
