...
func TestExtractImageRepo(t *testing.T) {
	tests := []struct {
		...
		{"synthorg digest", "    image: ghcr.io/aureliolo/synthorg-backend@sha256:abc", true, "ghcr.io/aureliolo/synthorg-backend"},
		{"synthorg tag", "    image: ghcr.io/aureliolo/synthorg-backend:0.7.3", true, "ghcr.io/aureliolo/synthorg-backend"},
		{"DHI nats with embedded port-like tag", "    image: dhi.io/nats:2.12-debian13@sha256:abc", true, "dhi.io/nats"},
		{"DHI postgres tag-only", "    image: dhi.io/postgres:18-debian13", true, "dhi.io/postgres"},
		{"registry with port", "    image: localhost:5000/synthorg-backend:latest", true, "localhost:5000/synthorg-backend"},
		{"registry with port and digest", "    image: localhost:5000/synthorg-backend@sha256:abc", true, "localhost:5000/synthorg-backend"},
		{"registry port differs", "    image: localhost:6000/synthorg-backend:latest", true, "localhost:6000/synthorg-backend"},
		{"tabs indent", "\timage: ghcr.io/aureliolo/synthorg-backend:0.7.3", true, "ghcr.io/aureliolo/synthorg-backend"},
		{"trailing comment", "    image: ghcr.io/foo/bar:tag  # pinned", true, "ghcr.io/foo/bar"},
		{"empty value", "    image:", false, ""},
		{"missing image keyword", "    foo: bar", false, ""},
		{"comment line", "# image: ghcr.io/foo:tag", false, ""},
		{"yaml array reject", "    image: [a, b]", false, ""},
		{"empty line", "", false, ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, ok := extractImageRepo(tt.line)
			if ok != tt.wantOK {
				t.Fatalf("extractImageRepo(%q) ok = %v, want %v", tt.line, ok, tt.wantOK)
			}
			if got != tt.wantSub {
				t.Errorf("extractImageRepo(%q) repo = %q, want %q", tt.line, got, tt.wantSub)
			}
		})
	}
}
