...
// skipCLIUpdate is set by reexecUpdate so the re-launched binary skips the
// redundant "Checking for updates..." / "CLI is up to date" messages.
...
func init() {
	updateCmd.Flags().BoolVar(&skipCLIUpdate, "skip-cli-update", false, "skip CLI self-update check (used internally after re-exec)")
