...
// tag/digest bumps where the repository is unchanged. Pinned-image bumps
// for both synthorg images and DHI base images (nats, postgres) are
// template-driven (Renovate digests embedded in the CLI), so the user has
// nothing to review.
func isUpdateBoilerplateOnly(existing, fresh []byte) bool {
	...
	for i := range oldLines {
		...
		if oldRepo, ok1 := extractImageRepo(oldLines[i]); ok1 {
			if newRepo, ok2 := extractImageRepo(newLines[i]); ok2 && oldRepo == newRepo {
				continue
			}
		}
		return false
...
// extractImageRepo returns the repository portion of a compose `image:` line
// (e.g. "ghcr.io/aureliolo/synthorg-backend" or "dhi.io/nats") and true if
// the line is a recognisable image declaration. Returns ("", false) for
// non-image lines.
func extractImageRepo(line string) (string, bool) {
