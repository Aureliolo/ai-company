...
func isUpdateBoilerplateOnly(existing, fresh []byte) bool {
	...
	for i := range oldLines {
		...
		if oldRepo, oldHasTag, oldHasDigest, ok1 := extractImageRepo(oldLine); ok1 {
			if newRepo, newHasTag, newHasDigest, ok2 := extractImageRepo(newLine); ok2 &&
				oldRepo == newRepo &&
				oldHasTag == newHasTag &&
				oldHasDigest == newHasDigest {
				continue
			}
		}
		return false
