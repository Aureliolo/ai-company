...
// tag/digest bumps where the repository AND the presence of a pin
// (`:tag` / `@digest`) are unchanged. Pinned-image bumps for both
// synthorg images and DHI base images (nats, postgres) are template-driven
// (Renovate digests embedded in the CLI), so the user has nothing to
// review. Pin removal or addition is intentionally NOT auto-applied: it
// changes the deployment's trust posture and must reach the user prompt.
func isUpdateBoilerplateOnly(existing, fresh []byte) bool {
	...
	for i := range oldLines {
		// Compose files written on Windows can carry CRLF line endings,
		// which leaves a trailing "\r" on each split chunk. Strip it
		// before comparison so the predicate behaves identically across
		// platforms (the regex below also rejects "\r" in its trailer).
		...
		if oldRepo, oldPinned, ok1 := extractImageRepo(oldLine); ok1 {
			if newRepo, newPinned, ok2 := extractImageRepo(newLine); ok2 &&
				oldRepo == newRepo && oldPinned == newPinned {
				continue
			}
		}
		return false
...
// whether a `:tag` or `@digest` pin was present, and whether the line is a
// recognisable image declaration at all. Returns ("", false, false) for
// non-image lines. Callers that auto-apply boilerplate must compare the
// pinned flag, not just the repository, because dropping a digest pin is
// a substantive trust change.
func extractImageRepo(line string) (repo string, pinned, ok bool) {
	sub := genericImageLinePattern.FindStringSubmatch(line)
	if len(sub) < 4 {
		return "", false, false
	}
	return sub[2], sub[3] != "", true
}
