...
- **`changelog_view`**: enum, either `highlights` (default) or `commits`. Sets the default view for the `synthorg update` upgrade walk between installed and target releases. `highlights` shows the AI-generated three-section summary (PR #1555+); `commits` shows the Release Please commit-based changelog. Inside the walk, `c` toggles between the two views for the current session without modifying the persisted value. Ignored on the `dev` channel: dev pre-releases have no Highlights block, so the walk renders a single combined commit list via the GitHub compare API instead.
