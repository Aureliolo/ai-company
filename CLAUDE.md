...
- **Pluggable subsystems**: new cross-cutting subsystems (e.g. classification detectors, context loaders, notification sinks, retention policies, verification graders) follow a protocol + strategy + factory + config discriminator pattern. Define a `Protocol` interface, ship concrete strategies that implement it, register them in a factory keyed by a config discriminator, and plumb the active selection through frozen config. Ship safe defaults so the behaviour is opt-in, never a silent regression. Canonical examples: `engine/classification/protocol.py` (`Detector`, `ScopedContextLoader`, `ClassificationSink`) + `budget/coordination_config.py` dispatcher; `engine/quality/decomposer_protocol.py` (`CriteriaDecomposer`) + `engine/quality/grader_protocol.py` (`RubricGrader`) + `engine/quality/verification_factory.py` + `engine/quality/verification_config.py`.
