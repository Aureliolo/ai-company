...
src/synthorg/
  ...
  observability/  # Structured logging (8-sink pipeline: console + 7 file sinks with logger-name routing), correlation tracking (request_id/task_id/agent_id via contextvars), sensitive field redaction, SYNTHORG_LOG_LEVEL env var override, critical sink enforcement (audit.log/access.log), third-party logger taming (_tame_third_party_loggers: clears LiteLLM/httpx/httpcore/aiosqlite/uvicorn/anyio/multipart handlers, sets WARNING + propagate=True, suppresses litellm.set_verbose to eliminate duplicate Docker log output), log sinks, events/degradation.py (FALLBACK/QUEUE/ALERT event constants)
