...
export function MeetingTokenBreakdown({ meeting, className }: MeetingTokenBreakdownProps) {
  ...
  // Build segments from participant usage
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {meeting.contribution_rank.map((agentId, i) => {
            const tokens = meeting.token_usage_by_participant[agentId] ?? 0
            const share = getParticipantTokenShare(meeting, agentId)
            const rankLabel = i < RANK_BADGES.length ? RANK_BADGES[i] : null
              ...
              <div key={agentId} className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 h-1 w-full overflow-hidden rounded-full bg-border">
                    <div
                      className={cn(
                        'h-full rounded-full bg-accent transition-all duration-[900ms]',
                      )}
                      style={{
                        width: `${share}%`,
