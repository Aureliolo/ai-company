...
**Async-leak ceiling (MANDATORY)**: CI's `Dashboard Test` job runs `vitest run --coverage --detect-async-leaks` and fails if the `Leaks N leaks` summary line reports more than `MAX_ASYNC_LEAKS` (currently 50) OR if the summary line is missing entirely. The 50-leak floor is structural: it comes from MSW 2.x's own `CookieStore` (tough-cookie), the MSW XHR interceptor's `queueMicrotask` dispatch, and axios's internal interceptor `promise.then` chain -- none reachable from `test-setup.tsx`. Lower the ceiling only after genuinely eliminating leaks at the source; never raise it. Full investigation + options matrix lives in `docs/design/web-http-adapter.md`. `test-setup.tsx` also installs a synchronous `document.cookie` shim on `Document.prototype` to bypass jsdom's tough-cookie Promise-based accessor: preserve that shim, and if a unit test needs a different cookie behavior override via `Document.prototype.cookie` at the test level (see `__tests__/utils/csrf.test.ts`) so the test's `afterEach` restores the shim by default.
