...
class TestAccuracyEffortRatio:
    ...
    def test_valid_ratio(self) -> None:
        ...
        assert ratio.accuracy == pytest.approx(0.8)
        assert ratio.effort == 1.0
        assert ratio.ratio == pytest.approx(0.8)
    ...
    def test_computed_ratio(self) -> None:
        ...
        assert ratio.accuracy == pytest.approx(0.6)
    ...
    def test_frozen(self) -> None:
        ...
        with pytest.raises(ValidationError):
            ratio.effort = 2.0  # type: ignore[misc]
    ...
    def test_accuracy_is_computed(self) -> None:
        """Accuracy is derived from correct_steps / total_steps."""
        ratio = AccuracyEffortRatio(
            effort=1.0,
            correct_steps=2,
            neutral_steps=1,
        ...
        assert ratio.accuracy == pytest.approx(2.0 / 3.0)
