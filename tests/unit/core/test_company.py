...
class TestDepartment:
    def test_valid_department(self, sample_department: Department):
...
class TestCompanyConfig:
    ...
    def test_empty_communication_pattern_rejected(self):
        with pytest.raises(ValidationError):
...
@pytest.mark.unit
class TestCompany:
    def test_valid_company(self, sample_company: Company):
    ...
    def test_budget_sum_at_100_accepted(self):
        depts = (
    ...
    def test_budget_sum_under_100_accepted(self):
        depts = (
    ...
    def test_budget_sum_over_100_rejected(self):
        depts = (
            Department(name="A", head="a", budget_percent=60.0),
    ...
    def test_empty_departments_accepted(self):
        co = Company(name="Empty")
        assert co.departments == ()

    def test_frozen(self, sample_company: Company):
        with pytest.raises(ValidationError):
            sample_company.name = "Changed"  # type: ignore[misc]

    def test_model_copy_update(self, sample_company: Company):
        updated = sample_company.model_copy(update={"name": "New Corp"})
        assert updated.name == "New Corp"
        assert sample_company.name == "Test Corp"

    def test_json_roundtrip(self, sample_company: Company):
