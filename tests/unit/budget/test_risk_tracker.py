...
def _make_risk_record(
    *,
    ...
    return RiskRecord(
        ...
        risk_score=score,
        risk_units=risk_units if risk_units is not None else score.risk_units,
...
class TestRiskTrackerRecord:
    ...
    async def test_record_and_count(self) -> None:
        ...
        await tracker.record(_make_risk_record())
    ...
    async def test_multiple_records(self) -> None:
        ...
        for i in range(5):
            await tracker.record(
                _make_risk_record(agent_id=f"agent-{i}"),
            )
...
class TestRiskTrackerQueries:
    ...
    async def test_get_total_risk(self) -> None:
        tracker = RiskTracker()
        await tracker.record(_make_risk_record(risk_units=0.5))
        await tracker.record(_make_risk_record(risk_units=0.3))
    ...
    async def test_get_agent_risk(self) -> None:
        tracker = RiskTracker()
        await tracker.record(
            _make_risk_record(agent_id="a", risk_units=0.5),
        )
        await tracker.record(
            _make_risk_record(agent_id="b", risk_units=0.3),
        )
        await tracker.record(
            _make_risk_record(agent_id="a", risk_units=0.2),
        )
    ...
    async def test_get_task_risk(self) -> None:
        tracker = RiskTracker()
        await tracker.record(
            _make_risk_record(task_id="t1", risk_units=0.4),
        )
        await tracker.record(
            _make_risk_record(task_id="t2", risk_units=0.6),
        )
    ...
    async def test_time_range_filter(self) -> None:
        ...
        await tracker.record(_make_risk_record(risk_units=0.5, timestamp=old))
        await tracker.record(_make_risk_record(risk_units=0.3, timestamp=now))
        total = await tracker.get_total_risk(
    ...
    async def test_get_records_all(self) -> None:
        tracker = RiskTracker()
        await tracker.record(_make_risk_record(agent_id="a"))
        await tracker.record(_make_risk_record(agent_id="b"))
    ...
    async def test_get_records_filtered_by_task(self) -> None:
        tracker = RiskTracker()
        await tracker.record(_make_risk_record(task_id="t1"))
        await tracker.record(_make_risk_record(task_id="t2"))
    ...
    async def test_get_records_filtered_by_action_type(self) -> None:
        tracker = RiskTracker()
        await tracker.record(
            _make_risk_record(action_type="code:write"),
        )
        await tracker.record(
            _make_risk_record(action_type="code:read"),
        )
...
class TestRiskTrackerPruning:
    ...
    async def test_prune_expired(self) -> None:
        ...
        await tracker.record(_make_risk_record(timestamp=old))
        await tracker.record(_make_risk_record(timestamp=now))
