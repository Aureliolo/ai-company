...
# -- Built-in template department policies (#723) ─────────────────
...
def _get_dept(config: RootConfig, name: str) -> Department:
    ...
    for d in config.departments:
        if d.name == name:
            return d
    pytest.fail(f"Department {name!r} not found")


def _action_types(dept: Department) -> set[str]:
    """Extract action_type values from a department's approval chains."""
    return {c.action_type for c in dept.policies.approval_chains}
...
class TestBuiltinDepartmentPolicies:
    ...
    def test_dev_shop_engineering_review_requirements(self) -> None:
        config = render_template(load_template("dev_shop"))
        eng = _get_dept(config, "engineering")
        assert eng.policies.review_requirements.min_reviewers == 1

    def test_dev_shop_qa_test_coverage_chain(self) -> None:
        config = render_template(load_template("dev_shop"))
        qa = _get_dept(config, "quality_assurance")
        assert "test_coverage" in _action_types(qa)
        chain = next(
            c for c in qa.policies.approval_chains if c.action_type == "test_coverage"
        )
        assert "QA Lead" in chain.approvers

    def test_dev_shop_operations_default_policies(self) -> None:
        config = render_template(load_template("dev_shop"))
        ops = _get_dept(config, "operations")
        assert ops.policies.approval_chains == ()

    # -- product_team ──────────────────────────────────────────────

    def test_product_team_engineering_review_requirements(self) -> None:
        config = render_template(load_template("product_team"))
        eng = _get_dept(config, "engineering")
        assert eng.policies.review_requirements.min_reviewers == 1

    def test_product_team_design_review_chain(self) -> None:
        config = render_template(load_template("product_team"))
        design = _get_dept(config, "design")
        assert "design_review" in _action_types(design)
        chain = next(
            c
            for c in design.policies.approval_chains
            if c.action_type == "design_review"
        )
        assert "UX Designer" in chain.approvers

    # -- agency ────────────────────────────────────────────────────

    def test_agency_engineering_review_requirements(self) -> None:
    ...
    def test_agency_operations_client_approval_chain(self) -> None:
        config = render_template(load_template("agency"))
        ops = _get_dept(config, "operations")
        assert "client_approval" in _action_types(ops)
        chain = next(
            c
            for c in ops.policies.approval_chains
            if c.action_type == "client_approval"
        )
        assert "Project Manager" in chain.approvers

    # -- full_company ──────────────────────────────────────────────

    def test_full_company_engineering_review_requirements(self) -> None:
        config = render_template(load_template("full_company"))
        eng = _get_dept(config, "engineering")
        assert eng.policies.review_requirements.min_reviewers == 2

    def test_full_company_engineering_code_review_chain(self) -> None:
        config = render_template(load_template("full_company"))
        eng = _get_dept(config, "engineering")
        assert "code_review" in _action_types(eng)
        chain = next(
            c for c in eng.policies.approval_chains if c.action_type == "code_review"
        )
        assert chain.approvers == ("Software Architect", "CTO")

    def test_full_company_security_review_chain(self) -> None:
        config = render_template(load_template("full_company"))
        sec = _get_dept(config, "security")
        assert "security_review" in _action_types(sec)
        chain = next(
            c
            for c in sec.policies.approval_chains
            if c.action_type == "security_review"
        )
    ...
    def test_full_company_operations_change_management_chain(self) -> None:
        config = render_template(load_template("full_company"))
        ops = _get_dept(config, "operations")
        assert "change_management" in _action_types(ops)
        chain = next(
            c
            for c in ops.policies.approval_chains
            if c.action_type == "change_management"
        )
