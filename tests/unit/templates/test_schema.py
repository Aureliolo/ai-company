...
class TestTemplateVariable:
    ...
    def test_required_without_default_accepted(self):
...
class TestTemplateDepartmentConfig:
    ...
    def test_budget_percent_over_100_rejected(self):
        with pytest.raises(ValidationError):
...
class TestTemplateMetadata:
    def test_valid_minimal(self):
        m = TemplateMetadata(name="Test", company_type="custom")
        assert m.name == "Test"
    ...
    def test_blank_name_rejected(self):
        with pytest.raises(ValidationError):
            TemplateMetadata(name="", company_type="custom")
...
class TestCompanyTemplate:
    def test_valid_minimal(self, make_template_dict):
    ...
    def test_agent_count_below_min_rejected(self, make_template_dict):
    ...
    def test_agent_count_above_max_rejected(self, make_template_dict):
    ...
    def test_duplicate_variable_names_rejected(self, make_template_dict):
    ...
    def test_duplicate_department_names_rejected(self, make_template_dict):
    ...
    def test_unique_variables_accepted(self, make_template_dict):
        t = CompanyTemplate(
            **make_template_dict(
                variables=(
                    {"name": "x"},
    ...
    def test_autonomy_out_of_range_rejected(self, make_template_dict):
        with pytest.raises(ValidationError):
            CompanyTemplate(**make_template_dict(autonomy=1.5))

    def test_negative_budget_rejected(self, make_template_dict):
        with pytest.raises(ValidationError):
            CompanyTemplate(**make_template_dict(budget_monthly=-10.0))

    def test_frozen(self, make_template_dict):
        t = CompanyTemplate(**make_template_dict())
        with pytest.raises(ValidationError):
