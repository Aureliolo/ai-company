...
class TestGetPersonalityPreset:
    def test_valid_preset_returns_dict(self):
...
class TestGenerateAutoName:
    def test_known_role_returns_from_pool(self):
        name = generate_auto_name("CEO", seed=0)
        assert isinstance(name, str)
        assert len(name) > 0

    def test_unknown_role_uses_default_pool(self):
    ...
    def test_whitespace_stripped_from_role(self):
        a = generate_auto_name("  CEO  ", seed=0)
        b = generate_auto_name("CEO", seed=0)
        assert a == b
