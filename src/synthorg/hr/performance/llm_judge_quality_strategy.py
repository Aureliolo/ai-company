...
and task metrics to a small LLM model from a different provider
family than the agent being scored.  Returns a structured JSON
...
class LlmJudgeQualityStrategy:
    ...
    async def score(
        ...
        *,
        ...
        On any provider or parsing failure, returns a zero-confidence
        fallback result.  Cost is recorded only on success.
    ...
    async def _record_cost(
        ...
        *,
        ...
        record = CostRecord(
            agent_id=agent_id,
            task_id=task_id,
            provider=NotBlankStr("quality-judge"),
